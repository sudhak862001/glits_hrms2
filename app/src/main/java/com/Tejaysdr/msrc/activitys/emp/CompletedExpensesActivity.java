...
import com.Tejaysdr.msrc.adapter.ExpenseBdeAdapter;
...
public class CompletedExpensesActivity extends AppCompatActivity {
    private static final String TAG = "";
    RecyclerView rv_expenselist;
    TextView tv_Norecords;
    Toolbar toolbar;
    ExpenseBdeAdapter expenseBdeAdapter;
    ArrayList<Expensesbdemodel> leadHistorylist = new ArrayList<>();
    Bundle val;
    private  String FCMToken;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_completed_expenses);
...
//        editor.commit();
        ...
        rv_expenselist = (RecyclerView)findViewById(R.id.rv_expenselist);
        tv_Norecords = (TextView)findViewById(R.id.errMsg);
        RecyclerView.LayoutManager mLayoutManager = new LinearLayoutManager(this);
        rv_expenselist.setLayoutManager(mLayoutManager);
        setTitle("Completed Expenses List");
    ...
    private void getReturnsInfo() {
        try {
            ...
            dialog.show();
            String url = BaseUrl.getBaseurl()+ "employee/completed_expenses_list_new";
                                    ...
                                    if (message.equalsIgnoreCase("success")) {
                                        ...
                                        while (keys.hasNext()) {
                                            ...
                                            if (!key.equalsIgnoreCase("message") && !key.equalsIgnoreCase("text") && !key.equalsIgnoreCase("token_expiry")) {
                                                Expensesbdemodel taggroupData = new Gson().fromJson(jsonObject.getJSONObject(key).toString(), new TypeToken<Expensesbdemodel>() {
                                                }.getType());
                                                leadHistorylist.add(taggroupData);
                                            }
                                        }
                                        expenseBdeAdapter = new ExpenseBdeAdapter(CompletedExpensesActivity.this, leadHistorylist, "pending");
                                        rv_expenselist.setAdapter(expenseBdeAdapter);
                                        expenseBdeAdapter.notifyDataSetChanged();
                                        tv_Norecords.setVisibility(View.GONE);
                                        rv_expenselist.setVisibility(View.VISIBLE);
                                        dialog.dismiss();
                                    } else {
                                        Toast.makeText(CompletedExpensesActivity.this, "No Data Found", Toast.LENGTH_SHORT).show();
                                        tv_Norecords.setVisibility(View.VISIBLE);
                                        rv_expenselist.setVisibility(View.GONE);
                                        dialog.dismiss();
                                    }
                                    dialog.dismiss();
                                } catch (Exception e) {
...
//                                Intent i = new Intent(ExpenseslListActivity.this, EMPLoginActivity.class);
